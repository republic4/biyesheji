...
class UserCf:
    ...
    def recommend(self, username, n=3):
        ...
        for user, score in dict(nearest_user).items():  # 最相近的n个用户
            for movies, scores in self.all_user[user].items():  # 推荐的用户的新能源汽车列表
...
def recommend_by_user_id(user_id):
    ...
    for user in users:
        ...
        if rates:
            for i in rates:
                rate.setdefault(str(i.movie.id), i.mark)#填充新能源汽车数据
    ...
    for un_watched_movie in un_watched:
        for watched_movie in watched:
            if un_watched_movie not in names:
                ...
                distances.append((similarity(un_watched_movie.id, watched_movie[0]) * watched_movie[1], un_watched_movie))#加入相似的新能源汽车
